...
def main() -> None:
    ...
    for gi, gamma in enumerate(gammas):
        for ei, eta_s in enumerate(etas):
            ...
            zz = qzzb_phase_bound(rho, cutoff, W, tau_points=tau_points, convention="paper_unsquared")
    ...
    pd.DataFrame(rows).to_csv(Path(cfg["paths"]["data"]) / "qzzb_certified_phase_diagram.csv", index=False)
    ...
    plt.xlabel("Signal transmittance eta_s")
    plt.ylabel("Accumulated phase diffusion Gamma")
    plt.title("QZZB-certified validity classification")
    savefig(Path(cfg["paths"]["figures"]) / "fig_qzzb_certified_classification.png")
